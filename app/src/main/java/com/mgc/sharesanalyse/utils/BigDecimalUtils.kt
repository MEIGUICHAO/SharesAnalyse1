...
object BigDecimalUtils {
    ...
    // 除法运算
    fun safeDiv(d1:Double,d2: Double):Double = if (d2>0) BigDecimal(d1).divide(BigDecimal(d2) ,4, BigDecimal.ROUND_HALF_UP).setScale(DECIMAL_POINT_NUMBER, BigDecimal.ROUND_HALF_UP).toDouble() else 0.toDouble()

    fun getPPBySafeDiv(d1:Double,d2: Double):Double = if (d2>0) BigDecimal(d1* Datas.NUM_100M).divide(BigDecimal(d2*Datas.NUM_WAN) ,4, BigDecimal.ROUND_HALF_UP).setScale(DECIMAL_POINT_NUMBER, BigDecimal.ROUND_HALF_UP).toDouble() else 0.toDouble()
